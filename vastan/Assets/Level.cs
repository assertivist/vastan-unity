...
public class Level {
    ...
    public void load(string level) {
        TextAsset ta = (TextAsset)Resources.Load(level);
        XmlDocument doc = new XmlDocument();
        doc.LoadXml(ta.text);
        XmlNode mapnode = doc.DocumentElement;
        name = parse_string(mapnode, "name");
        tagline = parse_string(mapnode, "tagline");
        author = parse_string(mapnode, "author");
        description = parse_string(mapnode, "description");
    ...
    private string parse_string(XmlNode n, string key) {
    ...
    private float parse_float(XmlNode n, string key) {
        if (n.Attributes[key] == null) {
            return 0;
        }
